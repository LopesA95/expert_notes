...
export function NewNoteCard({ onNoteCreated }: NewNoteCardProps) {
  ...
  function handleStartRecording() {
    ...
    if (!isSpeechRecognitionAPIAvailable) {
      alert('Infelizmente seu navegador não suporta o reconhecimento de voz.')
      return
    }
    ...
    speechRecognition.onresult = (e) => {
      const transcript = Array.from(e.results).reduce((text, result) => {
  ...
  return (
    ...
    <Dialog.Root>
      <Dialog.Trigger className='rounded-md flex flex-col bg-slate-700 text-left p-5 gap-3 outline-none hover:ring-2 hover:ring-slate-600 focus-visible:ring-2 focus-visible:ring-lime-400'>
        ...
        <p className='text-sm leading-6 text-slate-400'>Grave uma nota que será convertida para texto automaticamente.</p>
      </Dialog.Trigger>
